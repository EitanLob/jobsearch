...
{
    send(sock, command.c_str(), command.length(), 0); //send data from command which act like a buffer, with the length of the string in command and with flag 0
    std::cout << "Sent: " << command;//print the data we send
    char buffer[1024] = {0};//buffer for server response
    recv(sock, buffer, 1024 - 1, 0);//read from socket to buffer
    std::cout << "Received: " << buffer;// print buffer
}

void sendEmployer(const string& jobdata)
{
    // try to Initialize Winsock and exit if failed
    WSADATA wsaData;
    ...
    if (sock == INVALID_SOCKET)
    {
        ...
        WSACleanup(); //free all the system data that allocate for socket
        return ;
    }
    // SMTP server details
    const char* smtpServer = "smtp.gmail.com"; // gmail SMTP server
    int smtpPort = 25;                         // Common SMTP ports: 25,with gmail 465 (SSL), 587
    // Resolve server address
    struct hostent* host = gethostbyname(smtpServer);
    if (!host)
    {
        std::cerr << "Failed to resolve SMTP server!" << std::endl;
        closesocket(sock); //free all the system data that allocate for socket
        WSACleanup();
        std::cout << "Failed to resolve SMTP " << std::endl;
        return ;
    }

    //add data to socket the operating system gave
    sockaddr_in serverAddr = {};
    serverAddr.sin_family = AF_INET;
    serverAddr.sin_port = htons(smtpPort);
    serverAddr.sin_addr.s_addr = *((unsigned long*)host->h_addr);
    // try to Connect to the server and exit if failed
    ...
    char buffer[1024] = {0};//zero buffer to read from
    recv(sock, buffer, 1024 - 1, 0); // Initial server response
    std::cout << "Server: " << buffer;	//print the server response


    // SMTP communication

    sendCommand(sock, "EHLO a\r\n");    // Identify the client
    //sendCommand(sock, "STARTTLS\r\n");
    std::cout << "for now all good " << std::endl;
    //need to add TLS
    sendCommand(sock, "AUTH PLAIN AGdyb3VwMXNjZTIwMjRAZ21haWwuY29tAHdxamIga2Z2eCB6enZoIHJtYmwg \r\n");        // email + password base 64 encoded
    //sendCommand(sock, "TVNfcjJITkI0QHRyaWFsLWpwemttZ3FqZDBtbDA1OXYubWxzZW5kZXIubmV0IA==\r\n"); // Replace with base64-encoded username
    //sendCommand(sock, "VWtoZHFyZmczZEx3WXVQQQ==\r\n"); // Replace with base64-encoded password
    sendCommand(sock, "MAIL FROM:<group1sce2024@gmail.com>\r\n");
    sendCommand(sock, "RCPT TO:<group1sce2024@gmail.com>\r\n");
    sendCommand(sock, "DATA\r\n");
    sendCommand(sock, "Subject: jobsearch project\r\n\r\n");
    sendCommand(sock, "I found your profile intresting for my job:.\r\n.\r\n"); // End with a single period on a line
    sendCommand(sock, jobdata );
...
void sendPass(const string& candidatePassword)
{
    // try to Initialize Winsock and exit if failed
    WSADATA wsaData;
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0)
    ...
    sendCommand(sock, "DATA\r\n");
    sendCommand(sock, "Subject: jobsearch project\r\n\r\n");
    sendCommand(sock, "your password is:.\r\n.\r\n"); // End with a single period on a line
    sendCommand(sock, candidatePassword );
    sendCommand(sock, "QUIT\r\n");
    ...
    while (true)
    {
        ...
        if (choice == 1) {
            ...
            } else
            {
                ...
                {
                    //sendEmployer(employerJobs[jobNumber]);
